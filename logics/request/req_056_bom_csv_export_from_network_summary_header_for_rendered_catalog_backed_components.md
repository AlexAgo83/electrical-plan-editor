...
- Include quantity aggregation and pricing fields (`unit price`, `line total`, and recommended overall total) based on catalog unit price.
...
- Recommended V1 output groups rows by `Catalog` item (not one row per instance), because BOMs are typically quantity-aggregated.
...
- Recommended V1 footer/summary rows:
  - `TOTAL (priced rows only)` amount
  - optional `Unpriced rows count`
